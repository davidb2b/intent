...
export function EditableList({ label, values, onChange, placeholder = "Adicionar item...", variant = "chips", minItems = 1, maxItems = 30 }: Props) {
  ...
  return <div className={`intent-edit-list intent-edit-list-${variant}`}>
    ...
    <div className="intent-edit-values">
      {values.map((value, index) => variant === "rows" ? <div className="intent-edit-row" key={`${value}-${index}`}>
        ...
        <button aria-label={`Remover ${value}`} disabled={values.length <= minItems} onClick={() => remove(index)} type="button"><X size={14} /></button>
      </div> : <span className={`intent-chip ${variant === "exclusions" ? "intent-chip-exclusion" : ""}`} key={`${value}-${index}`}>
        {value}<button aria-label={`Remover ${value}`} disabled={values.length <= minItems} onClick={() => remove(index)} type="button"><X size={12} /></button>
      </span>)}
    </div>
    {variant !== "rows" && values.length < maxItems && <div className="intent-add-row">
