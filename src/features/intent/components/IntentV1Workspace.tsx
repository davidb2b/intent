...
import { Building2, CheckCircle2, ChevronRight, CircleAlert, CreditCard, Eye, FlaskConical, Home, LayoutList, LoaderCircle, LogOut, Mail, Phone, Target, Users } from "lucide-react"
...
type WorkspaceView = "inicio" | "pessoas" | "contas" | "watchlist" | "icp" | "sim" | "custos"
...
export function IntentV1Workspace({ session }: { session: Session }) {
  ...
  const pageTitle = view === "inicio" ? "Início" : view === "pessoas" ? "Pessoas" : view === "contas" ? "Contas" : view === "watchlist" ? "Watchlist" : view === "icp" ? "ICP" : view === "sim" ? "Testar classificação" : "Custos"
  ...
  return <div className="intent-v1-shell">
    <aside className="intent-v1-sidebar">
      ...
      <p>Configuração</p>
      <nav aria-label="Configuração"><button className={view === "icp" ? "is-active" : ""} onClick={() => setView("icp")} type="button"><Target size={16} /><span>ICP</span><small>v{workspace?.latestIcp?.version ?? 1}</small></button><button className={view === "sim" ? "is-active" : ""} onClick={() => setView("sim")} type="button"><FlaskConical size={16} /><span>Testar classificação</span></button><button className={view === "custos" ? "is-active" : ""} onClick={() => setView("custos")} type="button"><CreditCard size={16} /><span>Custos</span><small>interno</small></button></nav>
    ...
    <main className="intent-v1-main">
      ...
      {view === "custos" && <CostsView summary={summary} creditLimit={creditLimit} />}
      {selectedPerson && <PersonDrawer comments={comments} markingClient={markingClient} onClose={() => setSelectedPerson(null)} onMarkClient={() => void confirmClient(selectedPerson)} onReveal={(type) => setRevealRequest({ person: selectedPerson, type })} person={selectedPerson} />}
    </main>
  </div>
}
...
function ClassificationTester({ input, busy, result, onChange, onSubmit }: { input: { evidence: string; personName: string; role: string; company: string }; busy: boolean; result: PreviewSignalResult | null; onChange: (field: keyof typeof input, value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <section className="intent-v1-content"><SectionHeading title="Testar classificação" subtitle="Cole uma evidência pública e veja como o perfil ideal ativo interpreta o sinal. O teste não salva uma pessoa nem publica um sinal." /><form className="intent-v1-panel intent-v1-classifier" onSubmit={onSubmit}><div className="intent-v1-form-grid"><label>Nome da pessoa<input value={input.personName} onChange={(event) => onChange("personName", event.target.value)} placeholder="Opcional" /></label><label>Cargo<input value={input.role} onChange={(event) => onChange("role", event.target.value)} placeholder="Ex.: CTO" /></label><label>Empresa e porte<input value={input.company} onChange={(event) => onChange("company", event.target.value)} placeholder="Ex.: Empresa · 1000+" /></label></div><label>Evidência pública<textarea required value={input.evidence} onChange={(event) => onChange("evidence", event.target.value)} placeholder="Cole aqui o comentário ou atividade pública que deseja avaliar." /></label><div className="intent-v1-classifier-actions"><small>A avaliação usa a mesma régua do ICP ativo e pode consumir crédito do plano.</small><Button disabled={busy || !input.evidence.trim()} type="submit">{busy ? <><LoaderCircle className="intent-spin" size={15} />Avaliando…</> : <><FlaskConical size={15} />Classificar evidência</>}</Button></div></form>{result && <section className="intent-v1-panel intent-v1-verdict" aria-live="polite"><div className="intent-v1-verdict-heading"><div><span className={`intent-v1-verdict-status is-${result.status}`}>{statusLabel[result.status]}</span><h3>{result.judgment.score}% de intenção</h3></div><span><CheckCircle2 size={18} />Avaliação concluída</span></div><div className="intent-v1-verdict-grid"><div><strong>Fit do cargo</strong><span>{result.fit.cargo}</span></div><div><strong>Fit do porte</strong><span>{result.fit.porte}</span></div><div><strong>Regra identificada</strong><span>{result.judgment.rule === "nenhuma" ? "Nenhuma regra ativa" : result.judgment.rule}</span></div></div><p>{result.fit.resumo}</p><blockquote>{result.judgment.evidence}</blockquote><small>Esta prévia não foi salva. O sinal só entra no radar quando vier de uma coleta pública aprovada.</small></section>}</section>
}
function CostsView({ summary, creditLimit }: { summary: SignalSummary | null; creditLimit: number }) { const executions = summary?.executionHistory ?? []; return <section className="intent-v1-content"><SectionHeading title="Custos" subtitle="Acompanhe o consumo registrado pela operação. Os valores abaixo vêm das execuções reais e não de estimativas visuais." /><div className="intent-v1-kpis"><Kpi label="Uso deste mês" value={summary?.monthlyCostUsd ?? 0} detail="Dólares registrados" /><Kpi label="Limite mensal" value={creditLimit} detail="Créditos disponíveis no plano" /><Kpi label="Execuções" value={executions.length} detail="Histórico recente" /><Kpi label="Próxima estimativa" value={summary?.estimatedNextCostUsd ?? 0} detail="Custo calculado para a próxima coleta" /></div><section className="intent-v1-panel intent-v1-table-wrap"><table className="intent-v1-table"><thead><tr><th>Execução</th><th>Origem</th><th>Status</th><th>Itens lidos</th><th>Custo</th><th>Início</th></tr></thead><tbody>{executions.map((execution) => <tr key={execution.id}><td>{execution.type === "descoberta" ? "Busca de conversas" : "Atualização de resultados"}</td><td>{execution.origin === "agendada" ? "Agendada" : execution.origin === "manual" ? "Manual" : "Não informado"}</td><td>{execution.status === "concluida" ? "Concluída" : execution.status === "rodando" ? "Em andamento" : "Não concluída"}</td><td>{execution.postsRead} posts · {execution.commentsRead} comentários · {execution.peopleNew} pessoas</td><td>{execution.costUsd.toLocaleString("pt-BR", { style: "currency", currency: "USD" })}</td><td>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(execution.startedAt))}</td></tr>)}</tbody></table>{executions.length === 0 && <Empty text="Ainda não há execuções registradas para esta operação." />}</section></section> }
...
function SourceRow({ source, busyId = null, onStatusChange }: { source: SignalSource; busyId?: string | null; onStatusChange?: (source: SignalSource, status: "monitorada" | "descartada") => void }) { const pending = source.status === "candidata"; return <article className="intent-v1-source"><div><strong>{source.name ?? "Fonte pública"}</strong><p>{source.posts} posts · {source.comments} interações públicas</p></div><div className="intent-v1-source-actions">{pending && onStatusChange ? <><Button disabled={busyId === source.id} onClick={() => onStatusChange(source, "monitorada")} size="sm">{busyId === source.id ? <><LoaderCircle className="intent-spin" size={13} />Acompanhando…</> : "Acompanhar"}</Button><Button disabled={busyId === source.id} onClick={() => onStatusChange(source, "descartada")} size="sm" variant="outline">Remover</Button></> : <span className={source.status === "monitorada" ? "is-active" : ""}>{source.status === "monitorada" ? "Acompanhando" : "Pendente"}</span>}</div></article> }
