...
export function IntentV1Workspace({ session }: { session: Session }) {
  ...
  const peopleWithSignals = useMemo(() => visiblePeople.filter((person) => person.comments > 0), [visiblePeople])
  const strongPeople = useMemo(() => peopleWithSignals.filter((person) => (person.intentScore ?? 0) >= 70 || person.intentStatus === "lead"), [peopleWithSignals])
  ...
  const peopleByPriority = useMemo(() => [...peopleWithSignals].sort((a, b) => (b.intentScore ?? 0) - (a.intentScore ?? 0) || b.comments - a.comments), [peopleWithSignals])
  ...
  const commentsByPerson = useMemo(() => new Map(comments.map((comment) => [comment.personId, comment])), [comments])
