...
export function IntentV1Workspace({ session }: { session: Session }) {
  ...
  return <div className="intent-v1-shell">
    <aside className="intent-v1-sidebar">
      ...
      <p>Workspace</p>
      <nav aria-label="Navegação principal">{primaryItems.map(({ id, label, icon: Icon }) => <button className={view === id ? "is-active" : ""} key={id} onClick={() => setView(id)} type="button"><Icon size={16} /><span>{label}</span>{id === "pessoas" && peopleWithSignals.length > 0 ? <small>{peopleWithSignals.length}</small> : id === "watchlist" && watchPages.length + watchPeople.length > 0 ? <small>{watchPages.length + watchPeople.length}</small> : null}</button>)}</nav>
      <p>Configuração</p>
