  ...
  async function confirmContactReveal() {
    ...
    try {
      ...
      setRevealedContacts((current) => ({
        ...current,
        [revealRequest.person.id]: { ...current[revealRequest.person.id], [revealRequest.type]: result.contact },
      }))
...
function PersonRow({ person, detailed = false, onReveal, revealedContacts }: { person: SignalPerson; detailed?: boolean; onReveal?: (type: RevealContactType) => void; revealedContacts?: Partial<Record<RevealContactType, string>> }) { return <article className="intent-v1-row"><span className="intent-v1-avatar">{initials(person.name)}</span><div><strong>{person.name}</strong><p>{personSubline(person)}</p>{detailed && <a href={person.linkedinUrl} rel="noreferrer" target="_blank">Ver perfil público</a>}{detailed && onReveal && <div className="intent-v1-contact-actions">{revealedContacts?.email ? <span>{revealedContacts.email}</span> : <button onClick={() => onReveal("email")} type="button"><Mail size={13} />Consultar e-mail</button>}{revealedContacts?.telefone ? <span>{revealedContacts.telefone}</span> : <button onClick={() => onReveal("telefone")} type="button"><Phone size={13} />Consultar telefone</button>}</div>}</div><aside><b>{person.comments}</b><small>{person.comments === 1 ? "sinal" : "sinais"}</small></aside></article> }
