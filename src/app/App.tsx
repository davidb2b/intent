...
type PeopleFilter = "all" | "observed" | "discovered"
...
function App() {
  ...
  const [companySector, setCompanySector] = useState("all")
  const [companySort, setCompanySort] = useState<CompanySort>("comments")
  ...
  const availableCompanySectors = getCompanySectors(signalCompanies)
  const visibleCompanies = filterCompanyResults(signalCompanies, { search: companySearch, sector: companySector, sort: companySort })
  const orderedSources = [...signalSources].sort((first, second) => {
    const statusPriority = (source: SignalSource) => source.status === "monitorada" ? 0 : 1
    const priority = statusPriority(first) - statusPriority(second)
    if (priority !== 0) return priority
    return (second.yield ?? -1) - (first.yield ?? -1) || second.comments - first.comments || first.name!.localeCompare(second.name!, "pt-BR")
  })
  ...
  function openCompanyFromPerson(companyName: string | null) {
    ...
    if (!company) return
    setCompanySearch("")
    setCompanySector("all")
    setSelectedCompanyId(company.id)
    navigate("companies")
  }

  const selectedPost = reviewPosts.find((post) => post.id === selectedPostId) ?? reviewPosts[0]
  const selectedCompany = visibleCompanies.find((company) => company.id === selectedCompanyId) ?? visibleCompanies[0]
  ...
  function exportCompanies() {
    downloadCsv("signal-lab-empresas.csv", [["Empresa", "Setor", "Porte", "Pessoas observadas", "Comentários", "LinkedIn"], ...visibleCompanies.map((company) => [company.name, company.sector ?? "", company.size ?? "", String(company.people), String(company.comments), company.linkedinUrl ?? ""])])
  }
